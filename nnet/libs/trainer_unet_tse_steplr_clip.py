...
class Trainer(object):
    def __init__(self,
        ...
        # 只要在初始化时做一次封装
        if len(self.gpuid) > 1:
            self.nnet_parallel = th.nn.DataParallel(self.nnet, device_ids=self.gpuid)
        else:
            self.nnet_parallel = self.nnet
    ...
    def run(self, train_loader, dev_loader, num_epochs=120):
        ...
        with th.cuda.device(self.gpuid[0]):
            ...
            while self.cur_epoch < num_epochs:
                ...
                stats["scheduler"] = ""
                if cv["loss"] < best_loss:  # 只有 Loss 减小才是进步
                    best_loss = cv["loss"]
                    no_impr = 0
                    self.save_checkpoint(best=True)
                else:
                    no_impr += 1
                    stats["scheduler"] = "| no impr, best = {:.4f}".format(best_loss)
...
class SiSnrTrainer(Trainer):
	...
	std_m = th.std(egs["mix"], dim=-1, keepdim=True) + 1e-8
        # spks x n x S
        ests = self.nnet_parallel(egs["mix"], egs["aux"])
        target_len = egs["ref"].shape[-1]
