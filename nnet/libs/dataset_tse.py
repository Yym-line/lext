...
class Dataset(object):
    ...
    def __getitem__(self, index):
        ...
        #speaker = key.split('_')[0]+'.wav'
        mix = self.mix[key]
        aux = self.aux[key]   # enrollment (target speaker reference)
        ...
        if self.ref is not None:
            ref = self.ref[key]
            sample["ref"] = ref.astype(np.float32)
...
class ChunkSplitter(object):
    def __init__(self, chunk_size, train=True, least=2000, aux_chunk_size=32000): # 32000对应2s
        self.chunk_size = chunk_size  # 这里对应 Mix 的 4s
        self.aux_chunk_size = aux_chunk_size # 这里对应 Enroll 的 2s
        self.least = least
        self.train = train
    ...
    def _make_chunk(self, eg, s):
        ...
        aux_len = eg["aux"].shape[0]
        if aux_len > self.aux_chunk_size:
            # 训练时随机切 2s，验证时固定切前 2s
            start = random.randint(0, aux_len - self.aux_chunk_size) if self.train else 0
            chunk["aux"] = eg["aux"][start:start + self.aux_chunk_size]
        else:
            # 如果不足 2s 则补零
            P = self.aux_chunk_size - aux_len
            chunk["aux"] = np.pad(eg["aux"], (0, P), "constant")
